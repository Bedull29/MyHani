...
export default function TopTrailer({ mediaType }: TopTrailerProps) {
  ...
  */
  ...
  useEffect(() => {
    // Directly use the first mock movie for Hero to separate it from the "Popular" list logic
    import("src/mock/data").then(({ MOCK_MOVIES }) => {
      if (MOCK_MOVIES && MOCK_MOVIES.length > 0) {
        setVideo(MOCK_MOVIES[0]);
      }
    });
  }, []);
                  ...
                  <VideoJSPlayer
                    options={{
                      ...
                      sources: [
                        {
                          type: "video/youtube",
                          src: `https://www.youtube.com/watch?v=${detail.videos.results[0]?.key || "ZEoU8Rc0hHg"
                            }`,
                        },
                      ],
                    }}
