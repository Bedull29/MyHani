...
function SlideItem({ item, rank }: SlideItemProps) {
  return (
    <Box sx={{ pr: { xs: 0.5, sm: 1 } }}>
      <VideoItemWithHover video={item} rank={rank} />
    </Box>
  );
}
                ...
                {data.results
                  ...
                  .map((item, index) => (
                    <SlideItem
                      key={item.id}
                      item={item}
                      rank={genre.name.includes("10 Acara TV") ? index + 1 : undefined}
                    />
                  ))}
              </StyledSlider>
            </CustomNavigation>
